...
function clickOffset(image, event, xpixels, ypixels) 
{
	...
	if (ox===undefined) {
		...
		ox = event.clientX - p.x;
		oy = event.clientY - p.y;
	}
	var h = image.height;
	var w = image.width;
	// screen pixel offset
...
function bindTracking(imageName, callBackFunction, defaultString, xpixels, ypixels, dictionary) {
	...
	function mouseMove(e) {
      ...
      lastString = defaultString;
    }
	}
	try {
		image.addEventListener("mousemove", mouseMove, false);
		image.addEventListener("mouseout", mouseMove, false);
	} catch (e) {
		image.attachEvent("onmousemove", mouseMove);
		image.attachEvent("onmouseout", mouseMove);
	}
}
